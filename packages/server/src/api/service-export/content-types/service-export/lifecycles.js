...
module.exports = {
  async beforeCreate(event) {
    ...
    const svc = await strapi.db.query('api::service.service').findOne({where: {
      id: data.service
    }, populate: true});
    ...
    const k8sAxios = await strapi.service('api::kubernetes.kubernetes').getK8sAxios(svc.registry);
  ...
  async afterCreate() {
    ...
    for (let index = 0; index < regs.length; index++) {
      if (regs[index].type === 'k8s') {
        ...
        await strapi.service('api::service-import.service-import').syncCreateSvcImports(regs[index]);
      }
    }
  },
  ...
  async beforeDelete(event) {
    const serviceImport = await strapi.db.query('api::service-import.service-import').findOne({where: {
  ...
  async afterDelete(event) {
    ...
    // get all registries
    const regs = await strapi.db.query('api::registry.registry').findMany({populate: true});
    for (let index = 0; index < regs.length; index++) {
      if (regs[index].type === 'k8s') {
        // sync service imports from k8s
        await strapi.service('api::service-import.service-import').syncDeleteSvcImports(regs[index]);
      }
    }
  },
