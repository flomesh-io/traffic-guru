...
const systemSettingUploadConf = {
  ext: [".jpg", ".jpeg", ".png", ".zip"]
};
module.exports = {
  async init() {
    ...
    if (!uploadConf) {
      await strapi.db.query('api::system-setting.system-setting').create({
        data: {
          type: 'UploadConf',
          mode: 'global',
          content: systemSettingUploadConf,
        }
      });
    } else if (uploadConf.content.ext.length < 4) {
      await strapi.db.query('api::system-setting.system-setting').update({where:{id: uploadConf.id}, data:{content: systemSettingUploadConf}})
    }

    const mimeTypes = await strapi.db.query('api::system-setting.system-setting').findOne({where: {type: 'MimeTypes'}});
    if (!mimeTypes) {
      const systemSettingMimeTypes = {
      ...
      await strapi.db.query('api::system-setting.system-setting').create({
        data: {
          type: 'MimeTypes',
          mode: 'global',
          content: systemSettingMimeTypes,
        }
      });
    }
  }
