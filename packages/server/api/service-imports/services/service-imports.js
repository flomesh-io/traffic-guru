...
module.exports = {
  async syncCreateSvcImports(reg) {
    ...
    const k8sAxios = strapi.config.functions.kubeUtils.getK8sAxios(reg);
    ...
    await k8sAxios.get(svcImportsUrl).then(async function (response) {
      ...
      const guruSvcImports = await strapi.query('service-imports').find({
        registry: reg.id
      });
      for (let i = 0; i < k8sSvcImports.length; i++) {
        ...
        const svcList = await strapi.query('service').find({
          name: svcName,
          namespace: namespace
        });
        ...
        if (guruSvcImports.find((itemInGuru) => (itemInGuru.name == svcName && itemInGuru.namespace == namespace))) {
          ...
          strapi.query('service-imports').update({
            name: svcName,
            namespace: namespace,
            registry: reg.id, 
          }, {
          ...
          strapi.query('service-imports').create({
            name: svcName,
            namespace: namespace,
            registry: reg.id,
            serviceExports: serviceExports
    ...
    await k8sAxios.get(svcImportsUrl).then(async function (response) {
      ...
      for (let i = 0; i < guruSvcImports.length; i++) {
        ...
        if(serviceExports.length === 0){ // delete the svcImport if there is no other svcExports
          ...
          strapi.query('service-imports').delete({ id: itemInGuru.id });
  ...
  async createGlobalTrafficPolicy(svcImport) {
    ...
    const k8sAxios = strapi.config.functions.kubeUtils.getK8sAxios(svcImport.registry);
    ...
    if(lbTargets.length===0){
      throw new Error("At least one service export is required.");
    }
