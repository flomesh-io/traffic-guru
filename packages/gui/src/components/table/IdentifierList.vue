    ...
    >
      ...
      >
        <CardList
          ...
          :data-source="identifiers[_type.value] || []"
          :type="_type.value"
          :loading="false"
          :actions="[
...
export default {
  ...
  watch: {
    identifiersVals: {
      deep: true,
      handler() {
        this.$emit("update:identifiers", this.identifiersVals);
      },
    },
  ...
  methods: {
    ...
    remove(index, type) {
      this.identifiersVals = this.identifiers;
      this.identifiersVals[type].splice(index, 1);
      this.$emit("identifierSave");
    },
    ...
    handleOk() {
      this.identifiersVals = this.identifiers;
      this.identifiersVals[this.payload.type] = this.identifiersVals[
        this.payload.type
      ]
        ? this.identifiersVals[this.payload.type]
        : [];
      if (this.isEdit) {
        this.identifiersVals[this.payload.type][this.payload.index] = {
          name: this.payload.name,
          content: this.payload.content,
        };
      } else {
        this.identifiersVals[this.payload.type].push({
          name: this.payload.name,
          content: this.payload.content,
        });
      }
