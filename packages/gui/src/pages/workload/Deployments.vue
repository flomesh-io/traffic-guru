...
export default {
  ...
  methods: {
    detail(id, namespace) {
      this.$router.push({
        path: `/workload/deployment/detail/${namespace}/${id}`,
      });
    },
    ...
    URL() {
      ...
      return this.$REST.KUBE.encode(this.$REST.KUBE.DEPLOYMENT, append);
    },
    ...
    search(pageNo, pageSize) {
      ...
      this.$request(this.URL(), this.$METHOD.GET).then((res) => {
        let _data = res.data; 
        this.list = this.reset(_data.deployments); 
