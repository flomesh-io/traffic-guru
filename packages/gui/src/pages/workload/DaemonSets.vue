...
export default {
  name: "DaemonSets",
  components: { CPUMemory, DownOutlined },
  i18n: require("@/i18n"),
  data() {
    return {
      params: {
  ...
  methods: {
    detail(id, namespace) {
      this.$router.push({
        path: `/workload/daemonset/detail/${namespace}/${id}`,
      });
    },
    ...
    URL() {
      ...
      return this.$REST.KUBE.encode(this.$REST.KUBE.DAEMONSET, append);
    },
    ...
    search(pageNo, pageSize) {
      ...
      this.$request(this.URL(), this.$METHOD.GET).then((res) => {
        ...
        this.list = this.reset(_data.daemonSets); //res.data.data?res.data.data:[];
